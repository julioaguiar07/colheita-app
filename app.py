...
def get_clientes_consultor():
    """Retorna todos os clientes vinculados ao consultor com KPIs"""
    if request.usuario_role != 'consultor':
    ...
    try:
        ...
        resultado = []
        total_vendas_carteira = 0
        total_gastos_carteira = 0
        total_producoes_carteira = 0
        ...
        for cliente in rows:
            ...
            total_producoes = float(producoes['total'] or 0)
            ...
            total_vendas_carteira += total_vendas
            total_gastos_carteira += total_gastos
            total_producoes_carteira += total_producoes
            
            lucro = total_vendas - (total_producoes + total_gastos)
            ...
            resultado.append({
                ...
                'total_producoes_mes': total_producoes,
        ...
        total_custos_carteira = total_producoes_carteira + total_gastos_carteira
        total_lucro_carteira = total_vendas_carteira - total_custos_carteira
        
        return jsonify({
            'success': True, 
            'clientes': resultado,
            'resumo_carteira': {
                'total_vendas': total_vendas_carteira,
                'total_gastos': total_custos_carteira,
                'total_lucro': total_lucro_carteira,
...
@token_required
def criar_benchmark():
    """Retorna dados de benchmark baseado em margem de lucro"""
    if request.usuario_role != 'consultor':
        return jsonify({'error': 'Acesso negado'}), 403
    ...
    try:
        ...
        if len(vinculos) < 2:
            cur.close()
            conn.close()
            return jsonify({'success': False, 'error': 'Precisa de pelo menos 2 clientes vinculados'}), 200
        
        resultados = []
        clientes_com_vendas = 0
        ...
        for idx, vinculo in enumerate(vinculos):
            ...
            # Buscar custos (produções + gastos) dos últimos 3 meses
            cur.execute('''
                SELECT COALESCE(SUM(valor), 0) as total
                FROM gastos 
                WHERE usuario_id = %s 
            ...
            # Só adiciona se tiver vendas
            if total_vendas > 0:
                clientes_com_vendas += 1
        ...
        if len(resultados) < 2:
            return jsonify({
                'success': False, 
                'error': f'Apenas {clientes_com_vendas} cliente(s) com vendas. Adicione vendas para pelo menos 2 clientes.'
            }), 200
